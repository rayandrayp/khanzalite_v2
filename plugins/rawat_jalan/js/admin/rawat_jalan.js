...
$("#index").on("click", "#bukaform", function () {
  ...
  $("#form")
    .show()
    .load(baseURL + "/rawat_jalan/form?t=" + mlite.token);
...
$("#form").on("click", "#simpan", function (event) {
  ...
  if (no_rawat == "") {
    ...
    bootbox.alert(
      "Ada tagihan belum diselesaikan. Silahkan hubungi kasir atau admin!"
    );
  } else {
    $.post(
      url,
      ...
      function (data) {
        $("#display")
          .show()
          .load(baseURL + "/rawat_jalan/display?t=" + mlite.token);
        bersih();
        $("#status_pendaftaran").hide();
        $("#notif")
          .html(
            '<div class="alert alert-success alert-dismissible fade in" role="alert" style="border-radius:0px;margin-top:-15px;">' +
      ...
      $("#notif")
        .html(
          '<div class="alert alert-danger alert-dismissible fade in" role="alert" style="border-radius:0px;margin-top:-15px;">' +
...
$("#display").on("click", ".antrian", function (event) {
  ...
  window.open(
    baseURL + "/rawat_jalan/antrian?no_rawat=" + no_rawat + "&t=" + mlite.token
  );
});
...
$("#display").on("click", ".riwayat_perawatan", function (event) {
  ...
  window.open(
    baseURL + "/pasien/riwayatperawatan/" + no_rkm_medis + "?t=" + mlite.token
  );
});
      ...
      if (result) {
        ...
        $.post(
          ...
          function (data) {
            ...
            $("#display").load(
              baseURL + "/rawat_jalan/display?t=" + mlite.token
            );
            bersih();
            $("#notif")
              .html(
...
$("#display").on("click", ".sep", function (event) {
  ...
  $.get(url, function (data) {
    ...
    if (!json_obj[0]) {
      ...
      $("#status_peserta").text(
        json_obj[0].response.peserta.statusPeserta.keterangan
      );
      $("#jenis_peserta").text(
        json_obj[0].response.peserta.jenisPeserta.keterangan
      );
      $(".prolainis_peserta").text(
        json_obj[0].response.peserta.informasi.prolanisPRB
      );
      ...
      $("input:text[name=sep_jenis_kelamin_kode]").val(
        json_obj[0].response.peserta.sex
      );
      $("input:text[name=sep_tanggal_lahir]").val(
        json_obj[0].response.peserta.tglLahir
      );
      $("input:text[name=sep_jenis_peserta]").val(
        json_obj[0].response.peserta.jenisPeserta.keterangan
      );
      $("input:text[name=sep_no_kartu]").val(
        json_obj[0].response.peserta.noKartu
      );
      ...
      $("input:text[name=sep_kelas_kode]").val(
        json_obj[0].response.peserta.hakKelas.kode
      );
      $("input:text[name=sep_kelas_nama]").val(
        json_obj[0].response.peserta.hakKelas.keterangan
      );
      $("input:text[name=sep_nomor_telepon]").val(
        json_obj[0].response.peserta.mr.noTelepon
      );
    } else {
...
$("#manage").on("click", "#submit_periode_rawat_jalan", function (event) {
  ...
  var periode_rawat_jalan_akhir = $(
    "input:text[name=periode_rawat_jalan_akhir]"
  ).val();
  ...
  $.post(
    url,
    {
      periode_rawat_jalan: periode_rawat_jalan,
      periode_rawat_jalan_akhir: periode_rawat_jalan_akhir,
    },
    function (data) {
      // tampilkan data
      $("#form").show();
...
//       nm_pasien + ", nomor antrian " + no_reg + ", ke " + nm_poli,
//       "Indonesian Male",
//       { pitch: 1, rate: 0.8, volume: 2 }
//     );
//   }
//   play();
// });
...
$("#form_soap").on("click", "#odontogram", function (event) {
  ...
  var loadURL =
    baseURL + "/rawat_jalan/odontogram/" + id_pasien + "?t=" + mlite.token;
