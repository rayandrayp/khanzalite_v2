...
class Admin extends AdminModule
{
    ...
    public function getRawatJalan()
    {
        ...
        $settings = htmlspecialchars_array($this->settings('manajemen'));
        $dataRalanHariIni = $this->getDataPasienRalan(date('Y-m-d'));
        $dataRalanKemarin = $this->getDataPasienRalan(date('Y-m-d', strtotime('-1 days')));
        // var_dump($dataRalan);
        $ralanHariIniSemua = $dataRalanHariIni[0]['Belum'] + $dataRalanHariIni[0]['Sudah'] + $dataRalanHariIni[0]['Batal'];
        $ralanKemarinSemua = $dataRalanKemarin[0]['Belum'] + $dataRalanKemarin[0]['Sudah'] + $dataRalanKemarin[0]['Batal'];
        ...
        $stats['pasienBatal'] = number_format($dataRalanHariIni[0]['Batal'], 0, '', '.');
        $stats['percentpasienBatal'] = 0;
        if ($stats['pasienBatal'] != 0) {
            $stats['percentpasienBatal'] = number_format((100 + ((($dataRalanHariIni[0]['Batal'] - $dataRalanKemarin[0]['Batal']) / $dataRalanHariIni[0]['Batal']) * 100)), 0, '', '.');
        }
        $stats['pasienSudah'] = number_format($dataRalanHariIni[0]['Sudah'], 0, '', '.');
        $stats['percentpasienSudah'] = 0;
        if ($stats['pasienSudah'] != 0) {
            $stats['percentpasienSudah'] = number_format((($dataRalanHariIni[0]['Sudah'] / $ralanHariIniSemua) * 100), 0, '', '.');
        }
    ...
    public function countKunjungan()
    {
        ...
        $query = $this->db()->pdo()->prepare("SELECT 
                    ...
                    SUM(CASE WHEN tgl_registrasi = '$hari2' THEN 1 ELSE 0 END) harilalu
    ...
    public function getDataPasienRalan($date)
    {
        $query = $this->db()->pdo()->prepare("SELECT 
                    IFNULL(SUM(CASE WHEN stts <> 'Belum' OR stts <> 'Batal' THEN 1 ELSE 0 END),0) Belum,
                    IFNULL(SUM(CASE WHEN stts = 'Sudah' THEN 1 ELSE 0 END),0) Sudah,
                    IFNULL(SUM(CASE WHEN stts = 'Batal' THEN 1 ELSE 0 END),0) Batal
                FROM reg_periksa WHERE tgl_registrasi = '$date'");
        // $query = $this->db()->pdo()->prepare("SELECT 
